...
class Game:
    ...
    def load_start_assets(self):
        try:
            self.start_bg = pygame.image.load(f'{ASSET_FOLDER_PATH}/background/start.png')
            self.start_bg = pygame.transform.scale(self.start_bg, (SCREEN_WIDTH, SCREEN_HEIGHT))

            self.inital_bg = pygame.image.load(f'{ASSET_FOLDER_PATH}/background/initalize.png')
            self.inital_bg = pygame.transform.scale(self.inital_bg, (SCREEN_WIDTH, SCREEN_HEIGHT))
            ...
            scale_factor = 0.8  
            self.title_img = pygame.transform.scale(self.title_img, 
                                                  (int(title_width * scale_factor), 
                                                   int(title_height * scale_factor)))
            self.title_rect = self.title_img.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
