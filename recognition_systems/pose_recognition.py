...
class GestureRecognizer:
    ...
    def detect_punch(self, pose_landmarks):
        ...
        angle_confidence = min(1.0, max(0.0, (elbow_angle - 130) / 50))  # Normalize to 0-1 range
    ...
    def detect_kick(self, pose_landmarks):
        ...
        angle_confidence = min(1.0, max(0.0, (knee_angle - 130) / 50))  # Normalize to 0-1 range
        height_confidence = min(1.0, max(0.0, height_diff * 10))  # Scale height difference
    ...
    def detect_block(self, pose_landmarks):
        ...
        height_confidence = min(1.0, max(0.0, (0.15 - (height_diff_left + height_diff_right) / 2) / 0.15))
        
        # Combined confidence
        confidence = (left_angle_confidence * 0.3 + 
                     right_angle_confidence * 0.3 + 
                     height_confidence * 0.4)
        
        is_block = (left_elbow_angle < 120 and right_elbow_angle < 120 and 
...
class PoseResultCallback:
    ...
    def __call__(self, result, output_image, timestamp_ms):
        ...
        if result and result.pose_landmarks and len(result.pose_landmarks) > 0:
            self.detected_gesture, self.gesture_confidence = self.gesture_recognizer.detect_gesture(result.pose_landmarks[0])
        else:
            self.detected_gesture = None
            self.gesture_confidence = 0.0
    ...
    def get_gesture(self):
        ...
        if self.detected_gesture:
            data = {
                "pose": self.detected_gesture.lower(),
...
class PoseRecognition:
    ...
    def __init__(self, model_path='./models/pose/pose_landmarker_full.task', camera_id=0, 
                 detection_confidence=0.5, tracking_confidence=0.5, 
                 display_camera=True, display_landmarks=True):
        """
    ...
    def _run_loop(self):
        ...
        while self.is_running and self.cap.isOpened():
            ...
            if self.pose_callback.result and self.pose_callback.result.pose_landmarks and self.display_landmarks:
                # Draw pose landmarks using the Mediapipe solution draw method
                for idx, landmarks in enumerate(self.pose_callback.result.pose_landmarks):
            ...
            if self.display_camera:
                ...
                if self.pose_callback.detected_gesture:
                    ...
                    cv2.putText(image, gesture_text, (10, 70),
    ...
    def get_gesture(self):
        ...
        return self.pose_callback.get_gesture()
    ...
    def is_detecting_pose(self):
        ...
        return self.pose_callback.result is not None and len(self.pose_callback.result.pose_landmarks) > 0
...
def example_usage():
    ...
    try:
        ...
        while True:
            ...
            gesture_data = pose_system.get_gesture()
            
            # Print the JSON data for demonstration
            if gesture_data["pose"]:
                print(f"Gesture data: {gesture_data}")
            
            # Update game state based on gesture
            if gesture_data["pose"] and gesture_data["pose"] != last_gesture:
                print(f"Detected: {gesture_data['pose']} with confidence {gesture_data['confidence']}")
                score += int(gesture_data["confidence"] * 10)  # Score based on confidence
                last_gesture = gesture_data["pose"]
